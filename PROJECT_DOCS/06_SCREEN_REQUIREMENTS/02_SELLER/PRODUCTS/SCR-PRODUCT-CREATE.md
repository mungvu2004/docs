# SCR-PRODUCT-CREATE - Create Product

## Metadata

| Field | Value |
|---|---|
| Screen ID | SCR-PRODUCT-CREATE |
...
| Related Flow IDs | FLOW-PRODUCT-CREATE |
...
Tạo sản phẩm mới trong phạm vi shop hiện tại.
...
| --- | --- | --- |
| RULE-PRODUCT-CREATE-001 | Màn hình chỉ hiển thị dữ liệu theo đúng role và scope. | FLOW-PRODUCT-CREATE |
...
| --- | --- | --- |
| VAL-PRODUCT-CREATE-001 | User phải có quyền truy cập màn hình. | Bạn không có quyền truy cập màn hình này. |
| VAL-PRODUCT-CREATE-002 | Dữ liệu phải thuộc đúng shop/tenant hoặc platform scope. | Không thể truy cập dữ liệu ngoài phạm vi được cấp quyền. |
...
| --- | --- | --- | --- |
| Q-SCR-PRODUCT-CREATE-001 | Danh sách field product Lazada hỗ trợ sync chính xác là gì? | Ảnh hưởng form và sync status. | Pending API Verification |
...
|---|---|---|
| Screen có Screen ID chưa? | Pass | SCR-PRODUCT-CREATE |
| Có route chưa? | Pass | `/app/products/new` |
...
| Có map flow chưa? | Pass | FLOW-PRODUCT-CREATE |
