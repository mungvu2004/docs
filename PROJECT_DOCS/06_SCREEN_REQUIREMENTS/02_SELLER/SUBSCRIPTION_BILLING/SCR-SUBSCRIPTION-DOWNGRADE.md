# SCR-SUBSCRIPTION-DOWNGRADE - Downgrade Plan

## Metadata

| Field | Value |
|---|---|
| Screen ID | SCR-SUBSCRIPTION-DOWNGRADE |
| Screen Name | Downgrade Plan |
| Route | `/app/subscription/downgrade` |
| Screen Type | Seller |
| Module ID | MOD-SUBSCRIPTION |
| Related Feature IDs | FEAT-SUBSCRIPTION-VIEW-PLAN, FEAT-SUBSCRIPTION-PLAN-LIMITS, FEAT-SUBSCRIPTION-UPGRADE, FEAT-SUBSCRIPTION-DOWNGRADE, FEAT-SUBSCRIPTION-PAYMENT, FEAT-SUBSCRIPTION-INVOICE, FEAT-SUBSCRIPTION-STATUS |
| Related Flow IDs | FLOW-SUBSCRIPTION-UPGRADE-PAYMENT, FLOW-SUBSCRIPTION-DOWNGRADE, FLOW-SUBSCRIPTION-INVOICE-RECORD, FLOW-SUBSCRIPTION-PASTDUE-SUSPEND |
...
Hạ gói dịch vụ và kiểm tra impact giới hạn.
...
| --- | --- | --- | --- | --- |
| Current plan | Gói hiện tại và giới hạn. | Subscription/Plan | Yes |  |
| Payment status | PendingPayment/Paid/Failed/Expired/NeedReview. | Payment Record | For payment screens | Backend xác minh. |
| VNPay QR | QR/payment URL/thời hạn. | VNPay Payment Request | For QR screen | Không tự quyết định success ở frontend. |
| Invoice records | Lịch sử thanh toán/hóa đơn. | Invoice/Payment Record | For invoice history |  |
...
| --- | --- | --- | --- | --- | --- |
| ACT-SUB-UPGRADE | Upgrade Plan | ROLE-SHOP-OWNER | Choose upgrade | Payment request VNPay QR được tạo. | FLOW-SUBSCRIPTION-UPGRADE-PAYMENT |
| ACT-SUB-DOWNGRADE | Downgrade Plan | ROLE-SHOP-OWNER | Confirm downgrade | Downgrade theo policy nếu không vượt giới hạn. | FLOW-SUBSCRIPTION-DOWNGRADE |
| ACT-PAY-CHECK | Check Payment Result | ROLE-SHOP-OWNER | Open result / callback return | Hiển thị trạng thái payment đã xác minh. | FLOW-SUBSCRIPTION-UPGRADE-PAYMENT |
...
| --- | --- | --- |
| RULE-SUBSCRIPTION-DOWNGRADE-001 | Màn hình chỉ hiển thị dữ liệu theo đúng role và scope. | FLOW-SUBSCRIPTION-UPGRADE-PAYMENT, FLOW-SUBSCRIPTION-DOWNGRADE, FLOW-SUBSCRIPTION-INVOICE-RECORD, FLOW-SUBSCRIPTION-PASTDUE-SUSPEND |
...
| --- | --- | --- |
| VAL-SUBSCRIPTION-DOWNGRADE-001 | User phải có quyền truy cập màn hình. | Bạn không có quyền truy cập màn hình này. |
| VAL-SUBSCRIPTION-DOWNGRADE-002 | Dữ liệu phải thuộc đúng shop/tenant hoặc platform scope. | Không thể truy cập dữ liệu ngoài phạm vi được cấp quyền. |
...
| --- | --- | --- | --- | --- |
| Upgrade Plan | Yes | No | N/A | Tạo payment request. |
| Downgrade Plan | Yes | No | N/A | Cần hiển thị impact giới hạn. |
...
| --- | --- | --- |
| Upgrade/downgrade/payment/invoice view | Yes for change/payment | actor, shopId, plan, paymentId, status, time |
...
| --- | --- |
| SCR-PAYMENT-VNPAY-QR | Thanh toán VNPay. |
| SCR-PAYMENT-RESULT | Kết quả thanh toán. |
| SCR-INVOICE-HISTORY | Lịch sử thanh toán. |
...
| --- | --- | --- | --- |
| Q-SCR-SUBSCRIPTION-DOWNGRADE-001 | Chi tiết thời gian hết hạn QR và chính sách retry payment là gì? | Ảnh hưởng UX payment. | Open |

---

## 20. Design Notes

Màn payment chỉ hiển thị trạng thái từ backend/API. Không được coi user quay lại trang success là thanh toán thành công nếu chưa xác minh.
...
|---|---|---|
| Screen có Screen ID chưa? | Pass | SCR-SUBSCRIPTION-DOWNGRADE |
| Có route chưa? | Pass | `/app/subscription/downgrade` |
| Có map feature chưa? | Pass | FEAT-SUBSCRIPTION-VIEW-PLAN, FEAT-SUBSCRIPTION-PLAN-LIMITS, FEAT-SUBSCRIPTION-UPGRADE, FEAT-SUBSCRIPTION-DOWNGRADE, FEAT-SUBSCRIPTION-PAYMENT, FEAT-SUBSCRIPTION-INVOICE, FEAT-SUBSCRIPTION-STATUS |
| Có map flow chưa? | Pass | FLOW-SUBSCRIPTION-UPGRADE-PAYMENT, FLOW-SUBSCRIPTION-DOWNGRADE, FLOW-SUBSCRIPTION-INVOICE-RECORD, FLOW-SUBSCRIPTION-PASTDUE-SUSPEND |
