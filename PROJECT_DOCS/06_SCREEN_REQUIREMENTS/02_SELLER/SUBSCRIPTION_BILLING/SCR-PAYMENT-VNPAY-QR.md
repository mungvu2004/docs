# SCR-PAYMENT-VNPAY-QR - VNPay QR Payment

## Metadata

| Field | Value |
|---|---|
| Screen ID | SCR-PAYMENT-VNPAY-QR |
| Screen Name | VNPay QR Payment |
| Route | `/app/subscription/payment/vnpay-qr` |
| Screen Type | Seller |
...
Hiển thị QR thanh toán VNPay cho subscription.
...
| --- | --- | --- |
| RULE-PAYMENT-VNPAY-QR-001 | Màn hình chỉ hiển thị dữ liệu theo đúng role và scope. | FLOW-SUBSCRIPTION-UPGRADE-PAYMENT, FLOW-SUBSCRIPTION-DOWNGRADE, FLOW-SUBSCRIPTION-INVOICE-RECORD, FLOW-SUBSCRIPTION-PASTDUE-SUSPEND |
...
| --- | --- | --- |
| VAL-PAYMENT-VNPAY-QR-001 | User phải có quyền truy cập màn hình. | Bạn không có quyền truy cập màn hình này. |
| VAL-PAYMENT-VNPAY-QR-002 | Dữ liệu phải thuộc đúng shop/tenant hoặc platform scope. | Không thể truy cập dữ liệu ngoài phạm vi được cấp quyền. |
...
| --- | --- | --- | --- |
| Q-SCR-PAYMENT-VNPAY-QR-001 | Chi tiết thời gian hết hạn QR và chính sách retry payment là gì? | Ảnh hưởng UX payment. | Open |
...
|---|---|---|
| Screen có Screen ID chưa? | Pass | SCR-PAYMENT-VNPAY-QR |
| Có route chưa? | Pass | `/app/subscription/payment/vnpay-qr` |
