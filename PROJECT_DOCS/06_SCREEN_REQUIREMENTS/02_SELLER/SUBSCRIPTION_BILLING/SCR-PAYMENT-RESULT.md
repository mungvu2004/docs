# SCR-PAYMENT-RESULT - Payment Result

## Metadata

| Field | Value |
|---|---|
| Screen ID | SCR-PAYMENT-RESULT |
| Screen Name | Payment Result |
| Route | `/app/subscription/payment/result` |
| Screen Type | Seller |
...
Hiển thị kết quả thanh toán Success/Failed/Pending/Expired/NeedReview.
...
| --- | --- | --- |
| RULE-PAYMENT-RESULT-001 | Màn hình chỉ hiển thị dữ liệu theo đúng role và scope. | FLOW-SUBSCRIPTION-UPGRADE-PAYMENT, FLOW-SUBSCRIPTION-DOWNGRADE, FLOW-SUBSCRIPTION-INVOICE-RECORD, FLOW-SUBSCRIPTION-PASTDUE-SUSPEND |
...
| --- | --- | --- |
| VAL-PAYMENT-RESULT-001 | User phải có quyền truy cập màn hình. | Bạn không có quyền truy cập màn hình này. |
| VAL-PAYMENT-RESULT-002 | Dữ liệu phải thuộc đúng shop/tenant hoặc platform scope. | Không thể truy cập dữ liệu ngoài phạm vi được cấp quyền. |
...
| --- | --- | --- | --- |
| Q-SCR-PAYMENT-RESULT-001 | Chi tiết thời gian hết hạn QR và chính sách retry payment là gì? | Ảnh hưởng UX payment. | Open |
...
|---|---|---|
| Screen có Screen ID chưa? | Pass | SCR-PAYMENT-RESULT |
| Có route chưa? | Pass | `/app/subscription/payment/result` |
