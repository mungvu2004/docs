# SCR-SHOP-USER-DETAIL - Shop User Detail

## Metadata

| Field | Value |
|---|---|
| Screen ID | SCR-SHOP-USER-DETAIL |
| Screen Name | Shop User Detail |
| Route | `/app/users/:userId` |
| Screen Type | Seller |
| Module ID | MOD-USER |
| Related Feature IDs | FEAT-USER-ASSIGN-ROLE, FEAT-USER-REMOVE-ROLE, FEAT-USER-DEACTIVATE-SHOP-USER |
...
Xem/gán/gỡ role hoặc khóa user trong shop.
...
| --- | --- | --- |
| RULE-SHOP-USER-DETAIL-001 | Màn hình chỉ hiển thị dữ liệu theo đúng role và scope. | FLOW-USER-INVITE-SHOP-USER |
...
| --- | --- | --- |
| VAL-SHOP-USER-DETAIL-001 | User phải có quyền truy cập màn hình. | Bạn không có quyền truy cập màn hình này. |
| VAL-SHOP-USER-DETAIL-002 | Dữ liệu phải thuộc đúng shop/tenant hoặc platform scope. | Không thể truy cập dữ liệu ngoài phạm vi được cấp quyền. |
...
| --- | --- | --- | --- |
| Q-SCR-SHOP-USER-DETAIL-001 | Permission chi tiết cho từng field/action sẽ chốt ở Permission Matrix thế nào? | Ảnh hưởng UI action visibility. | Open |
...
|---|---|---|
| Screen có Screen ID chưa? | Pass | SCR-SHOP-USER-DETAIL |
| Có route chưa? | Pass | `/app/users/:userId` |
| Có map feature chưa? | Pass | FEAT-USER-ASSIGN-ROLE, FEAT-USER-REMOVE-ROLE, FEAT-USER-DEACTIVATE-SHOP-USER |
