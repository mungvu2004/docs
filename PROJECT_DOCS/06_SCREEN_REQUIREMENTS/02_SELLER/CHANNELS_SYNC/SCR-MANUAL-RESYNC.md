# SCR-MANUAL-RESYNC - Manual Resync

## Metadata

| Field | Value |
|---|---|
| Screen ID | SCR-MANUAL-RESYNC |
| Screen Name | Manual Resync |
| Route | `/app/sync/manual-resync` |
| Screen Type | Seller |
...
Yêu cầu hoặc xác nhận manual resync dữ liệu.
...
| --- | --- | --- |
| RULE-MANUAL-RESYNC-001 | Màn hình chỉ hiển thị dữ liệu theo đúng role và scope. | FLOW-CHANNEL-CONNECT-LAZADA, FLOW-CHANNEL-DISCONNECT-LAZADA, FLOW-SYNC-ERROR-RETRY, FLOW-SYNC-MANUAL-RESYNC |
...
| --- | --- | --- |
| VAL-MANUAL-RESYNC-001 | User phải có quyền truy cập màn hình. | Bạn không có quyền truy cập màn hình này. |
| VAL-MANUAL-RESYNC-002 | Dữ liệu phải thuộc đúng shop/tenant hoặc platform scope. | Không thể truy cập dữ liệu ngoài phạm vi được cấp quyền. |
...
| --- | --- | --- | --- |
| Q-SCR-MANUAL-RESYNC-001 | Lazada API/webhook cụ thể hỗ trợ dữ liệu nào? | Ảnh hưởng data displayed và action retry/resync. | Pending API Verification |
...
|---|---|---|
| Screen có Screen ID chưa? | Pass | SCR-MANUAL-RESYNC |
| Có route chưa? | Pass | `/app/sync/manual-resync` |
