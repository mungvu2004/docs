# SCR-LAZADA-CHANNEL-DETAIL - Lazada Channel Detail

## Metadata

| Field | Value |
|---|---|
| Screen ID | SCR-LAZADA-CHANNEL-DETAIL |
| Screen Name | Lazada Channel Detail |
| Route | `/app/channels/lazada` |
| Screen Type | Seller |
| Module ID | MOD-CHANNEL, MOD-SYNC |
| Related Feature IDs | FEAT-CHANNEL-LIST, FEAT-CHANNEL-LAZADA-CONNECT, FEAT-CHANNEL-LAZADA-DISCONNECT, FEAT-CHANNEL-CONNECTION-STATUS, FEAT-CHANNEL-CONFIG, FEAT-SYNC-LOG, FEAT-SYNC-ERROR, FEAT-SYNC-RETRY, FEAT-SYNC-MANUAL-RESYNC-APPROVAL |
| Related Flow IDs | FLOW-CHANNEL-CONNECT-LAZADA, FLOW-CHANNEL-DISCONNECT-LAZADA, FLOW-SYNC-ERROR-RETRY, FLOW-SYNC-MANUAL-RESYNC |
| Allowed Roles | ROLE-SHOP-OWNER, ROLE-SHOP-MANAGER, ROLE-PRODUCT-STAFF limited |
| Priority | Should-have |
| Status | Draft |
| Depends On | 01_PROJECT_BRIEF.md, 02_USER_ROLES.md, 03_FEATURE_LIST.md, 04_BUSINESS_FLOW.md, 05_SITEMAP_AND_SCREEN_LIST.md |
| Used By | UI/UX Design, Frontend, Backend API, Permission Matrix, QA |

---

## 1. Screen Purpose

Xem trạng thái kết nối Lazada, cấu hình và ngắt kết nối nếu cần.

---

## 2. User Goal

User muốn xem hoặc xử lý nghiệp vụ liên quan đến màn hình này một cách rõ ràng, đúng quyền và đúng dữ liệu.

---

## 3. Entry Points

| Entry Point | From Screen | Condition |
| --- | --- | --- |
| Main navigation | Sidebar / Menu | User có quyền truy cập. |
| Related detail/action | Màn hình liên quan | User đi theo flow nghiệp vụ. |

---

## 4. Exit Points / Navigation

| Action | Target Screen | Condition |
| --- | --- | --- |
| Back / Breadcrumb | Previous screen | User quay lại màn trước. |
| Open related detail | Related screen | User có quyền xem dữ liệu liên quan. |

---

## 5. Allowed Roles & Access Rules

| Role ID | Access Level | Notes |
| --- | --- | --- |
| ROLE-SHOP-OWNER | View / Limited by permission | Theo permission matrix. |
| ROLE-SHOP-MANAGER | View / Limited by permission | Theo permission matrix. |
| ROLE-PRODUCT-STAFF limited | View / Limited by permission | Theo permission matrix. |

---

## 6. Main UI Sections

| Section ID | Section Name | Purpose | Visible To |
| --- | --- | --- | --- |
| SEC-HEADER | Header / Title | Hiển thị tên màn hình, breadcrumb và action chính. | Allowed roles |
| SEC-CONTENT | Main Content | Hiển thị dữ liệu chính của màn hình. | Allowed roles |
| SEC-STATE | State Area | Hiển thị loading, empty, error, permission denied. | Allowed roles |

---

## 7. Data Displayed

| Data Field | Description | Source / Related Entity | Required | Notes |
| --- | --- | --- | --- | --- |
| Channel status | Connected/AuthError/Disconnected/NotStarted. | Sales Channel | Yes |  |
| Sync type | Order/Product/Stock/Fees. | Sync Log | For sync screens |  |
| Sync result | Success/Error/Retrying/Failed. | Sync Log | For sync screens |  |
| Impact level | Low/High impact manual resync. | Sync Request | For manual resync | High impact cần Shop Owner xác nhận. |

---

## 8. User Actions

| Action ID | Action Name | Actor / Role | Trigger | Expected Result | Related Flow |
| --- | --- | --- | --- | --- | --- |
| ACT-LAZADA-CONNECT | Connect Lazada | ROLE-SHOP-OWNER, ROLE-SHOP-MANAGER | Start connection | Lazada channel connected hoặc lỗi rõ ràng. | FLOW-CHANNEL-CONNECT-LAZADA |
| ACT-LAZADA-DISCONNECT | Disconnect Lazada | ROLE-SHOP-OWNER | Confirm disconnect | Channel disconnected, sync jobs dừng. | FLOW-CHANNEL-DISCONNECT-LAZADA |
| ACT-SYNC-RETRY | Retry Sync | ROLE-SHOP-OWNER, ROLE-SHOP-MANAGER, ROLE-PRODUCT-STAFF | Retry error | Sync job retry theo policy. | FLOW-SYNC-ERROR-RETRY |
| ACT-MANUAL-RESYNC | Request Manual Resync | ROLE-SHOP-OWNER, ROLE-SHOP-MANAGER limited | Submit resync scope | Resync chạy hoặc chờ Shop Owner xác nhận nếu high impact. | FLOW-SYNC-MANUAL-RESYNC |

---

## 9. Action Rules

| Rule ID | Rule | Related Flow / Decision |
| --- | --- | --- |
| RULE-LAZADA-CHANNEL-DETAIL-001 | Màn hình chỉ hiển thị dữ liệu theo đúng role và scope. | FLOW-CHANNEL-CONNECT-LAZADA, FLOW-CHANNEL-DISCONNECT-LAZADA, FLOW-SYNC-ERROR-RETRY, FLOW-SYNC-MANUAL-RESYNC |

---

## 10. Form Fields

| Field Name | Type | Required | Validation | Placeholder / Example | Notes |
| --- | --- | --- | --- | --- | --- |
| Không áp dụng | - | - | - | - | Màn này không có form chính hoặc form sẽ được chi tiết ở bước UI/API. |

---

## 11. Validation Rules

| Validation ID | Rule | Error Message |
| --- | --- | --- |
| VAL-LAZADA-CHANNEL-DETAIL-001 | User phải có quyền truy cập màn hình. | Bạn không có quyền truy cập màn hình này. |
| VAL-LAZADA-CHANNEL-DETAIL-002 | Dữ liệu phải thuộc đúng shop/tenant hoặc platform scope. | Không thể truy cập dữ liệu ngoài phạm vi được cấp quyền. |

---

## 12. State Handling

### 12.1 Loading State

Hiển thị skeleton/loading phù hợp với bảng, form hoặc detail. Disable action chính khi đang xử lý.

### 12.2 Empty State

Hiển thị thông báo rõ ràng khi chưa có dữ liệu, kèm CTA nếu user có quyền tạo dữ liệu.

### 12.3 Error State

Hiển thị lỗi dễ hiểu, có nút thử lại nếu lỗi có thể retry.

### 12.4 Permission Denied State

Hiển thị thông báo user không có quyền truy cập hoặc không có quyền thực hiện action.

### 12.5 Success State

Hiển thị toast hoặc inline message khi thao tác thành công.

---

## 13. Confirmation / Approval

| Action | Confirm Required | Approval Required | Approver | Notes |
| --- | --- | --- | --- | --- |
| Disconnect Lazada | Yes | No | N/A | Dừng sync, giữ dữ liệu cũ. |
| Manual Resync High Impact | Yes | Yes | Shop Owner | Áp dụng khi ảnh hưởng trạng thái đơn, tiền, sản phẩm, tồn kho, dữ liệu nhạy cảm. |

---

## 14. Notification / Toast

| Event | Message Type | Suggested Message |
| --- | --- | --- |
| Load failed | Error | Không thể tải dữ liệu. Vui lòng thử lại. |
| Action success | Success | Thao tác thành công. |

---

## 15. Audit Log Requirement

| Action | Audit Required | Data to Log |
| --- | --- | --- |
| Connect/disconnect/retry/manual resync | Yes | actor, channelId/syncId, action, impact, result, time |

---

## 16. Responsive Requirement

| Device | Requirement |
|---|---|
| Desktop | Ưu tiên layout đầy đủ: table/filter/detail/action. |
| Tablet | Giữ đầy đủ chức năng chính, có thể thu gọn filter/action. |
| Mobile | Ưu tiên đọc thông tin và action quan trọng; table có thể chuyển thành card/list. |

---

## 17. Accessibility Notes

- Tất cả button/action phải có label rõ ràng.
- Form field phải có label và error message.
- Trạng thái loading/error/success cần có text, không chỉ màu sắc.
- Có focus state cho keyboard navigation.
- Không dùng màu làm cách duy nhất để truyền đạt trạng thái.

---

## 18. Related Screens

| Screen ID | Relationship |
| --- | --- |
| SCR-PRODUCT-DETAIL | Product sync status. |
| SCR-ORDER-DETAIL | Order sync status. |
| SCR-INVENTORY-OVERVIEW | Stock sync. |

---

## 19. Open Questions

| Question ID | Question | Impact | Status |
| --- | --- | --- | --- |
| Q-SCR-LAZADA-CHANNEL-DETAIL-001 | Lazada API/webhook cụ thể hỗ trợ dữ liệu nào? | Ảnh hưởng data displayed và action retry/resync. | Pending API Verification |

---

## 20. Design Notes

Thiết kế cần ưu tiên sự rõ ràng, dễ scan dữ liệu, action chính nổi bật, tránh quá nhiều thông tin phụ trên cùng một màn.

---

## 21. Dev Notes

- Cần enforce permission ở backend, không chỉ ẩn nút trên frontend.
- Cần truyền tenant/shop scope trong mọi request seller-side.
- Các API lỗi phải trả message đủ để UI hiển thị trạng thái đúng.
- Các action nhạy cảm cần confirm/audit theo flow.

---

## 22. Verification Checklist

| Check Item | Result | Notes |
|---|---|---|
| Screen có Screen ID chưa? | Pass | SCR-LAZADA-CHANNEL-DETAIL |
| Có route chưa? | Pass | `/app/channels/lazada` |
| Có map feature chưa? | Pass | FEAT-CHANNEL-LIST, FEAT-CHANNEL-LAZADA-CONNECT, FEAT-CHANNEL-LAZADA-DISCONNECT, FEAT-CHANNEL-CONNECTION-STATUS, FEAT-CHANNEL-CONFIG, FEAT-SYNC-LOG, FEAT-SYNC-ERROR, FEAT-SYNC-RETRY, FEAT-SYNC-MANUAL-RESYNC-APPROVAL |
| Có map flow chưa? | Pass | FLOW-CHANNEL-CONNECT-LAZADA, FLOW-CHANNEL-DISCONNECT-LAZADA, FLOW-SYNC-ERROR-RETRY, FLOW-SYNC-MANUAL-RESYNC |
| Có allowed roles chưa? | Pass | ROLE-SHOP-OWNER, ROLE-SHOP-MANAGER, ROLE-PRODUCT-STAFF limited |
| Có action chính chưa? | Pass | Đã có user actions. |
| Có state handling chưa? | Pass | Loading/empty/error/permission/success. |
| Có validation nếu có form chưa? | Pass | Đã có validation hoặc ghi không áp dụng. |
| Có open questions nếu thiếu thông tin chưa? | Pass | Đã ghi open questions. |
